...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.22.5' || $lastCheckPackage !== 'Ext' )
		{
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.22.5' ), '2.22.5' ) > 0 )
					{
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.22.5.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
		...
		if( ( $switch || version_compare( $verLast, '2.22.5' ) > 0 ) && !isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
		...
		if( isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
			...
			if( !$switch && version_compare( $verLast, '2.22.5' ) > 0 )
...
class PluginRe
{
	...
	static function Exec( $sMethod, $sUrl, $aHdrs, $nWaitForEnd = true, $bBody = true, &$sBody = null )
	{
		...
		$sCmd = Gen::ExecEscArg( $sPhpLauncher ) . ' -c ' . Gen::ExecEscArg( php_ini_loaded_file() ) . ' -d opcache.file_cache=' . Gen::ExecEscArg( $dirOpCache ) . ' -d opcache.enable_cli=1 -d opcache.file_cache_only=1 -d opcache.optimization_level=' . ini_get( 'opcache.optimization_level' ) . ' -d display_errors=' . ini_get( 'display_errors' ) . ' -d output_buffering=' . ini_get( 'output_buffering' ) . ' -d memory_limit=' . ini_get( 'memory_limit' ) . ' -d max_execution_time=' . ini_get( 'max_execution_time' ) . ' -d register_argc_argv=1 ' . Gen::ExecEscArg( dirname( __DIR__ ) . '/re.php' ) . ( is_int( $nWaitForEnd ) ? ( ' --t=' . $nWaitForEnd ) : '' ) . ( $fileOutTmt ? ( ' --tmp=' . Gen::ExecEscArg( $fileOutTmt ) ) : '' ) . ' --root=' . Gen::ExecEscArg( str_replace( '\\', '/', $_SERVER[ 'DOCUMENT_ROOT' ] ) ) . ' --soft=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( rawurlencode( (isset($_SERVER[ 'SERVER_SOFTWARE' ])?$_SERVER[ 'SERVER_SOFTWARE' ]:'') ) ) ) . ' --method=' . Gen::ExecEscArg( $sMethod ) . ' --url=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( $sUrl ) ) . ' --hdrs=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( rawurlencode( @json_encode( ( array )$aHdrs ) ) ) );
